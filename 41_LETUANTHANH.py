...
# NNhân viên lớn tuổi nhất của các PhongBan : 
cursor.execute('''
    SELECT PhongBan, HoTen, Tuoi
    FROM NhanVien
    WHERE Tuoi = (SELECT MAX(Tuoi) FROM NhanVien AS sub WHERE sub.PhongBan = NhanVien.PhongBan)
''')
PB= cursor.fetchall()
...
# Xóa nhân viên có “MaSV = 2” rồi cho biết mỗi phòng ban có bao nhiêu người 
...
# Trình bày các bước kết nối đến SQLite trong Python và thực thi các câu lệnh trên bằng Python:
